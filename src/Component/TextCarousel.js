...
const TextCarousel = ({title,emplacement,desc}) => {
    ...
    return(
        <>
            <div className={`position-absolute ${(emplacement+1)%2===0 ? 'end-0' : 'start-0'} mx-2 ${clas()} pt-4 px-2 pb-1`}>
                <h3 className={`text-center color-aris mb-lg-5 mb-md-2 mb-sm-1 ${isSM ? 'fs-3' : isMD ? 'fs-3' : isLG ? 'fs-2' : isXS ? 'fs-6' : 'fs-1'}`}>{title}</h3>
                <p className={`${isSM ? 'fs-6' : isMD ? 'fs-5' : isLG ? 'fs-4' : isXS ? 'fs-7': 'fs-3 pt-3'} text-white text-center m-0 p-0 ms-2`}>
                    {desc}
                </p>
                <div className="p-2 pe-4 w-100 d-flex justify-content-end">
                    <button type="button" className="btn btn-light rounded-0">En savoir Plus <span className=""><i className="fa-solid fa-angles-right"/></span></button>
                </div>
            </div>
        </>
    )
